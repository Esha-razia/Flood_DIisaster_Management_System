        ...
        if DB_AVAILABLE:
            try:
                cursor.execute("""
                    ...
                    (location, rainfall, river_level, temperature, input_data, risk, confidence, explanation)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    data.get("location"),
                    rainfall,
...
def get_alerts():
    if DB_AVAILABLE:
        try:
            ...
            return jsonify(MEMORY_ALERTS[::-1])
    else:
        # Return in-memory alerts
        return jsonify(MEMORY_ALERTS[::-1])  # Reverse to show newest first
