...
export default function GovDashboard() {
  ...
  const fetchAllRescueOps = async () => {
    try {
      const res = await axios.get("http://127.0.0.1:5000/rescue-operations");
  ...
  const fetchDonations = async () => {
    try {
      const res = await axios.get("http://127.0.0.1:5000/donations");
  ...
  const fetchSeasonalTrend = async () => {
    try {
      const res = await axios.get("http://127.0.0.1:5000/admin/seasonal-trend");
  ...
  const handleAssignRescueWorker = async (alertItem) => {
    try {
      const opRes = await axios.post("http://127.0.0.1:5000/rescue-operations", {
      ...
      await axios.put(`http://127.0.0.1:5000/alerts/${alertItem.id}`, {
  ...
  const handleUpdateWorkerStatus = async (workerId, newStatus) => {
    try {
      ...
      await axios.put(`http://127.0.0.1:5000/users/${workerId}/${endpoint}`);
      fetchRescueWorkers();
    } catch (err) {
      console.error(err);
    }
