...
export default function CitizenDashboard() {
  ...
  // Once the citizen picks (or changes) their city, refresh alerts scoped
  // to that city — this is what makes "alerts in your area" actually mean
  // their area, instead of every alert system-wide.
  useEffect(() => {
    if (formData.location) fetchAlerts(formData.location);
  }, [formData.location]);
  ...
  const fetchPredictions = async () => {
    try {
      const res = await axios.get(`${API_BASE}/predictions`, { params: { email: userEmail } });
  ...
  const fetchAlerts = async (location) => {
    try {
      const res = await axios.get(`${API_BASE}/alerts`, { params: location ? { location } : {} });
      setAlerts(res.data || []);
    } catch (err) {
      console.error(err);
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(`${API_BASE}/predict`, { ...formData, user_email: userEmail });
